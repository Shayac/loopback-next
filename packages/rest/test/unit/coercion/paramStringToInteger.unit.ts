...
describe('coerce param from string to integer - optional', function() {
  ...
  context('integers larger than MAX_SAFE_INTEGER should trigger ERROR_BAD_REQUEST', async()=> {
    test(INTEGER_PARAM, '2343546576878989879789', RestHttpErrors.exceedsMaxSafeInt(REQUIRED_INTEGER_PARAM.name));
    test(INTEGER_PARAM, '-2343546576878989879789', RestHttpErrors.exceedsMaxSafeInt(REQUIRED_INTEGER_PARAM.name));
  })

  context('scientific notations', () => {
    test(INTEGER_PARAM, '1.234e+3', 1.234e+3);
    test(INTEGER_PARAM, '-1.234e+3', -1.234e3);
  });

  context('integer-like string values should trigger ERROR_BAD_REQUEST', () => {
    test(INTEGER_PARAM, '1.2', RestHttpErrors.invalidData(1.2, INTEGER_PARAM.name));
    test(INTEGER_PARAM, '-1.2', RestHttpErrors.invalidData(-1.2, INTEGER_PARAM.name));
    test(INTEGER_PARAM, '1.234e+30', RestHttpErrors.invalidData(1.234e+30, INTEGER_PARAM.name));
    test(INTEGER_PARAM, '-1.234e+30', RestHttpErrors.invalidData(-1.234e+30, INTEGER_PARAM.name));    
  });
  ...
  context('empty values trigger ERROR_BAD_REQUEST', () => {
    ...
    test(
      INTEGER_PARAM,
      '',
      RestHttpErrors.missingRequired(INTEGER_PARAM.name),
    );
  });
